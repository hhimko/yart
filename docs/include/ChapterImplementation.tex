...
As seen in \cref{lst:Implementation/RayGeneration/ray_generation}, our implementation of ray generation depends on just a couple of variables: the camera's looking direction, field of view, and the output image's size.
...
\end{equation}
%
where $ I_s $ is the light source intensity, and $ k_d $ is the surface's local diffuse coefficient.
...
For scenarios when the ray misses every geometry, a specialized procedure of rendering the world's background is usually implemented. 
...
For the sake of simplicity, the presented implementation will render solid color skies, which are defined by a single color value, independent of the ray's direction.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[height=9cm]{res/RayTracingLoopRender.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[height=11cm]{res/RayTracingLoopRender_shadows.png}
