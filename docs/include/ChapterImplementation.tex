...
%
\begin{equation}
    \bm{C}_{\textnormal{\small world}} = (\bm{VP})^{-1} \cdot \bm{C}_{\textnormal{\small screen}} = (\bm{C}^{-1} \bm{V}^{-1}) \cdot \bm{C}_{\textnormal{\small screen}}
\end{equation}
...
\end{equation}
%
However, since our goal is to only calculate ray directions, we can assume the camera is positioned at the worlds's origin, and just focus on its orientation.
...
%
{
    ...
    \begin{align}
        right &= - \frac{forward\times\worldup}{\norm{forward\times\worldup}}\label{eq:Implementation/RayGeneration/right}\\[1em]
        up    &= - right \times forward\label{eq:Implementation/RayGeneration/up}
    \end{align}
    
}

The above equations aid us in implementing a view matrix creation function, seen in \cref{lst:Implementation/RayGeneration/view_matrix}.
...
Another concept worth defining is the \textit{projection matrix}, which in turn is meant for transforming
